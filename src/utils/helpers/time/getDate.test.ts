...
it('should return correct values for each property', () => {
  ...
  expect(result.meridiemHours.value).toBe(
    LOCAL_HOURS % 12 === 0 ? 12 : LOCAL_HOURS % 12,
  );
...
it('should return correct meridiem hours', () => {
  ...
  expect(getDate(new Date('1999-03-12 12:00:00')).meridiemHours.type).toBe(
    'pm',
  );

  expect(getDate(new Date('1999-03-12 11:00:00')).meridiemHours.value).toBe(11);
  expect(getDate(new Date('1999-03-12 11:00:00')).meridiemHours.type).toBe(
    'am',
  );

  expect(getDate(new Date('1999-03-12 00:00:00')).meridiemHours.value).toBe(12);
  expect(getDate(new Date('1999-03-12 00:00:00')).meridiemHours.type).toBe(
    'am',
  );
});
